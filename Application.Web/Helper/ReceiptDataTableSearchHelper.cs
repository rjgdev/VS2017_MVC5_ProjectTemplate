...
{
    public class ReceiptDataTableSearchHelper
    {
         private int _customerId = int.Parse(CookieHelper.CustomerId);

        public async System.Threading.Tasks.Task<dynamic> SearchFunction(ExpectedReceiptFilterViewModels request)
        {
            ...
            if (response.IsSuccessful)
            {
                ...
                list = JsonConvert.DeserializeObject<List<dynamic>>(result).Select(x => new ExpectedReceiptViewModel.ExpectedReceipt
                {
                    Id = x.Id,
                    IsActive = x.IsActive,
                ...
                }).Where(x => x.Planned == false).ToList();
                ...
                filteredList = list.Where(x =>
                                     ...
                                     //x.WarehouseCode.ToLower().Contains(searchBy) ||
                                     x.Address.ToLower().Contains(searchBy) ||
                                     //x.Supplier.ToString().ToLower().Contains(searchBy) ||
                                     x.HaulierName.ToLower().Contains(searchBy));
                                     //x.Comments.ToString().ToLower().Contains(searchBy));
            }
